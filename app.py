...
# ====== 数据爬取（天天基金 API，更稳定） ======
def fetch_nav_data():
    """从天天基金获取历史净值，兼容 Render 环境"""
    url = "https://fundgz.1234567.com.cn/js/{}.js".format(FUND_CODE)
    try:
        # 先获取最新估值（不一定用到，但可以验证基金存在）
        headers = {"User-Agent": "Mozilla/5.0", "Referer": "https://fund.eastmoney.com/"}
        r = requests.get(url, headers=headers, timeout=10)
        r.encoding = 'utf-8'
        if r.status_code != 200:
            raise Exception("天天基金API请求失败")
    except:
        pass  # 即使估值接口失败，历史净值接口可能仍可用

    # 使用天天基金历史净值接口（JSONP，但兼容性好）
    ...
    try:
        ...
        if not items:
            raise Exception("返回数据为空，可能IPO太短或接口变更")
        records = []
        for item in items:
            records.append({
                "date": item["FSRQ"],
...
def api_all():
    ...
    if df is None or len(df) < 120:
        return jsonify({"error": "无法获取足够的历史数据，请检查数据源或基金代码"})
    result = train_and_backtest(df)
    if result[0] is None:
        return jsonify({"error": "训练数据不足，请稍后再试"})
